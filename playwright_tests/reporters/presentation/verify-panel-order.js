...
async page => {
  ...
  if(await page.getByLabel('Min seconds',{exact:true}).count()||await page.getByLabel('Max seconds',{exact:true}).count()) throw Error('Duration controls remain');
  if(await page.locator('#test-list-table th:visible').filter({hasText:/^Project$/}).count()) throw Error('Project column remains');
