...
async function check(mode) {
  ...
  const status = {};
  const button = { addEventListener: (_, fn) => { click = fn; }, closest: () => ({ querySelector: selector => selector === 'a[download]' ? { href: 'http://localhost/perfetto.json', download: 'perfetto.json' } : status }) };
  ...
  const context = {
    ...
    AbortController,
    fetch: async (_, options) => mode === 'stalled-fetch'
    ...
    setInterval: () => 1, clearInterval() {}, setTimeout: fn => { timeout = fn; return 2; }, clearTimeout() {}
  };
