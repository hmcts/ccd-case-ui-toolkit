...
async page => {
  ...
  if (await feature.evaluate(el => el.parentElement.firstElementChild === el) !== true) throw new Error('Feature overview not prioritised');
  ...
  await page.locator('.report-test-scroll').evaluate(el => { el.scrollTop = 200; });
  ...
  await page.locator('.report-test-scroll').evaluate(el => { el.scrollTop = 0; });
