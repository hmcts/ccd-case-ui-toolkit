...
const source = fs.readFileSync(`${__dirname}/report.js`, 'utf8').split('// Trace Viewer loads')[1].split('// Perfetto\'s documented')[0];
for (const [baseURI, href] of [
  ...
  const panel = parse(`<div id="TabTrace-test">${href ? `<a class="download-btn" download="trace.zip" href="${href}">Download Trace</a><a href="#">View Trace</a>` : ''}</div>`).firstChild;
  const download = panel.querySelector('a[download]');
  if (download) {
    download.href = href;
    download.after = node => download.insertAdjacentHTML('afterend', `<a class="${node.className}" href="${node.href}" target="${node.target}" rel="${node.rel}">${node.textContent}</a>`);
    download.parentElement = panel;
    panel.append = node => panel.insertAdjacentHTML('beforeend', `<p>${node.textContent}</p>`);
  }
  panel.append = node => panel.insertAdjacentHTML('beforeend', `<p>${node.textContent}</p>`);
  vm.runInNewContext('// Trace Viewer loads' + source, { URL, document: {
    baseURI, querySelectorAll: () => [panel], createElement: () => ({})
  } });
  const open = panel.querySelector('.trace-open');
  assert.equal(Boolean(open), Boolean(href));
  if (!href) { assert.match(panel.textContent, /No trace was retained/); continue; }
