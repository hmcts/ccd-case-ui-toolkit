...
test('accepts a corrected required value and preserves the optional value', async ({ page }) => {
  ...
  await page.getByRole('group', { name: 'Show conditional field' }).getByLabel('No').check();
  ...
  await page.getByRole('textbox', { name: 'Required field', exact: true }).fill('Complete this field');
...
test('shows, validates, hides and omits a conditional required field', async ({ page }) => {
  await page.goto('/');
