...
/*fdescribe('CaseFullAccessViewComponent organiseTabPosition (isolated)', () => {
  let fixture: ComponentFixture<CaseFullAccessViewComponent>;
  let component: CaseFullAccessViewComponent;
  const routerNavigateSpy = jasmine.createSpy('navigate').and.returnValue(Promise.resolve(true));

  class MockMatTab {
    constructor(public textLabel: string) {}
  }
  class MockMatTabGroup {
    constructor(private tabs: MockMatTab[]) {}
    get _tabs() {
      return {
        toArray: () => this.tabs,
        find: (fn: any) => this.tabs.find(fn)
      };
    }
    selectedIndex = 0;
  }
  ...
  const locationStub = {
    path: (_includeHash: boolean) => '/cases/case-details/J/PRLAPPS/12345' // ends with caseId (no tab id)
  };
  ...
  const activatedRouteStub = {
    snapshot: {
      data: {
        case: {
          case_id: '12345',
          case_type: { id: 'PRLAPPS', jurisdiction: { id: 'J' } },
          tabs: []
        }
      }
    },
  ...
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      declarations: [CaseFullAccessViewComponent],
      providers: [
        { provide: Router, useValue: { navigate: routerNavigateSpy } },
        { provide: Location, useValue: locationStub },
        { provide: ActivatedRoute, useValue: activatedRouteStub },
        FieldsUtils,
        NavigationNotifierService // <-- provide to satisfy DI in component
      ],
      schemas: [NO_ERRORS_SCHEMA]
    })
    // Override template to avoid translation pipe/child component dependencies
    .overrideTemplate(CaseFullAccessViewComponent, '<div></div>')
    .compileComponents();
    ...
    component.caseDetails = {
      ...
      case_type: { id: 'PRLAPPS', jurisdiction: { id: 'J' } },
      tabs: [
        { id: 'overview', label: 'Overview', order: 1, fields: [] },
        { id: 'summary', label: 'Summary', order: 2, fields: [] },
        { id: 'history', label: 'History', order: 3, fields: [] }
      ]
    } as any;
    component.tabGroup = new MockMatTabGroup([
      new MockMatTab('Overview'),
      new MockMatTab('Summary'),
      new MockMatTab('History')
    ]) as any;
    routerNavigateSpy.calls.reset();
    fixture.detectChanges();
  }));

  it('navigates to preferred tab fragment and selects it when rendered', async () => {
    await component.organiseTabPosition();

    expect(routerNavigateSpy).toHaveBeenCalledWith(
      ['cases', 'case-details', 'J', 'PRLAPPS', '12345'],
      { fragment: 'Summary' }
    );
    expect((component.tabGroup as any).selectedIndex).toBe(1);
  });

  it('falls back to default focus when preferred tab not rendered', async () => {
    component.tabGroup = new MockMatTabGroup([
      new MockMatTab('Overview'),
      new MockMatTab('History')
    ]) as any;
    fixture.detectChanges();
    const initialIndex = (component.tabGroup as any).selectedIndex;
    ...
    expect((component.tabGroup as any).selectedIndex).toBe(initialIndex);
  });

  it('selects custom tab when URL last segment matches appended tab id', async () => {
    const loc = TestBed.inject(Location) as any;
    spyOn(loc, 'path').and.returnValue('/cases/case-details/J/PRLAPPS/12345/tasks');

    component.appendedTabs = [{ id: 'tasks', label: 'Tasks', order: 99, fields: [] } as any];
    component.tabGroup = new MockMatTabGroup([
      new MockMatTab('Tasks'),
      new MockMatTab('Overview'),
      new MockMatTab('Summary')
    ]) as any;
    fixture.detectChanges();
    ...
    const lastCall = routerNavigateSpy.calls.mostRecent().args;
    expect(lastCall[0]).toEqual(['cases', 'case-details', 'J', 'PRLAPPS', '12345', 'tasks']);
    expect((component.tabGroup as any).selectedIndex).toBe(0);
  });

  it('does nothing (keeps default focus) if preferred tab label not found after navigation', async () => {
    component.caseDetails.tabs = [
      { id: 'overview', label: 'Overview', order: 1, fields: [] },
      { id: 'history', label: 'History', order: 2, fields: [] }
    ] as any;
    component.tabGroup = new MockMatTabGroup([
      new MockMatTab('Overview'),
      new MockMatTab('History')
