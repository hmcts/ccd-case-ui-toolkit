...
export class CaseFileViewFolderComponent implements OnInit, OnDestroy {
  ...
  @Output() public moveDocument = new EventEmitter<{
    newCategory: string;
  ...
  public nestedChildren = (_: number, nodeData: DocumentTreeNode) =>
    nodeData.children;
  ...
  public ngOnInit(): void {
    ...
    this.documentFilterFormGroup.addControl(
      CaseFileViewFolderComponent.DOCUMENT_SEARCH_FORM_CONTROL_NAME,
      this.documentSearchFormControl
    );

    // Listen to search input and initiate filter documents if at least three characters entered
    this.documentFilterSubscription =
      this.documentSearchFormControl.valueChanges
        .pipe(
          tap(
            (searchTerm: string) => (this.searchTermLength = searchTerm.length)
          ),
          switchMap((searchTerm: string) =>
            this.filter(searchTerm.toLowerCase()).pipe()
          )
        )
    ...
    this.categoriesAndDocumentsSubscription =
      this.categoriesAndDocuments.subscribe((categoriesAndDocuments) => {
        ...
        if (
          categoriesAndDocuments.uncategorised_documents &&
          categoriesAndDocuments.uncategorised_documents.length > 0
        ) {
        ...
        this.sortDataSourceDescending(
          CaseFileViewSortColumns.DOCUMENT_UPLOAD_TIMESTAMP
        );
      });
  }
  ...
  public generateTreeData(
    ...
    return categories.reduce((tree, node) => {
      ...
      newDocumentTreeNode.children = [
        ...this.generateTreeData(node.sub_categories),
        ...this.getDocuments(node.documents),
      ];
      newDocumentTreeNode.category_order = node.category_order;

      return [...tree, newDocumentTreeNode].sort(
  ...
  public getDocuments(documents: CaseFileViewDocument[]): DocumentTreeNode[] {
    ...
    documents.forEach((document) => {
      ...
      documentTreeNode.upload_timestamp =
        this.appConfig.getEnableCaseFileViewVersion1_1() &&
        document.upload_timestamp
          ? document.upload_timestamp.toString()
          : "";
  ...
  public getUncategorisedDocuments(
    uncategorisedDocuments: CaseFileViewDocument[]
  ): DocumentTreeNode {
    ...
    uncategorisedNode.name =
      CaseFileViewFolderComponent.UNCATEGORISED_DOCUMENTS_TITLE;
  ...
  public filter(searchTerm: string): Observable<DocumentTreeNode[]> {
    ...
    if (
      ...
      searchTerm.length >=
        CaseFileViewFolderComponent.MINIMUM_SEARCH_CHARACTERS &&
      this.documentFilterFormGroup.controls[
        CaseFileViewFolderComponent.DOCUMENT_SEARCH_FORM_CONTROL_NAME
      ].value.length > 0
    ) {
      filteredData = this.documentTreeData
        .map(copy)
        .filter(function filterTreeData(node: DocumentTreeNode) {
          if (
            node.name &&
            node.name.toLowerCase().includes(searchTerm) &&
            node.type === DocumentTreeNodeType.DOCUMENT
          ) {
            return true;
          }
  ...
  public triggerDocumentAction(
    ...
    switch (actionType) {
      case "changeFolder":
        ...
        break;
      case "openInANewTab":
        this.windowService.setLocalStorage(
      ...
      case "download":
        ...
        this.downloadFile(
          new URL(documentTreeNode.document_binary_url).pathname,
          documentTreeNode.document_filename
        );
        break;
      case "print":
        this.printDocument(
  ...
  public updateNodeData(data: DocumentTreeNode[]): void {
    ...
    const flattenedArray = this.nestedDataSource
      .map((item) => {
