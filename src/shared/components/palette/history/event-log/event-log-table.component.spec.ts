...
describe('EventLogTableComponent', () => {
  ...
  describe('Standalone use', () => {
    ...
    it('should render a row for each event', () => {
      ...
      expect(firstRowCells[COL_DATE].nativeElement.textContent).toBe('10 May 2017, 10:00:00 AM Local: ' + moment(EVENTS[0].timestamp).utcOffset(timeZoneOffset).format('D MMM YYYY, h:mm:ss A'));
      ...
      expect(secondRowCells[COL_DATE].nativeElement.textContent).toBe('9 May 2017, 4:07:03 PM Local: ' + moment(EVENTS[1].timestamp).utcOffset(timeZoneOffset).format('D MMM YYYY, h:mm:ss A'));
