...
import { ReadFieldsFilterPipe } from "../../palette";
...
let FieldReadComponent: any = MockComponent({
  ...
  inputs: ["caseField", "caseReference", "topLevelFormGroup"],
});
...
const $ERROR_MESSAGE_GENERIC = By.css(
...
xdescribe("CaseFullAccessViewComponent", () => {
  ...
  beforeEach(async(() => {
    ...
    alertService = createSpyObj("alertService", [
      "setPreserveAlerts",
      "success",
      "warning",
      "clear",
    ...
    matDialogRef = createSpyObj<MatDialogRef<DeleteOrCancelDialogComponent>>(
      "matDialogRef",
      ["afterClosed", "close"]
    );

    activityService = createSpyObj<ActivityPollingService>(
      "activityPollingService",
      ["postViewActivity"]
    );
    activityService.postViewActivity.and.returnValue(Observable.of());

    mockCallbackErrorSubject = createSpyObj<any>("callbackErrorSubject", [
      "next",
      "subscribe",
      "unsubscribe",
    ]);
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        CaseReferencePipe,
        {
          provide: NavigationNotifierService,
          useValue: navigationNotifierService,
        },
  ...
  it("should render each tab defined by the Case view", () => {
    ...
    expect(tabHeaders.find((c) => "Name" === attr(c, "title"))).toBeTruthy(
      "Could not find tab Name"
    );
    expect(tabHeaders.find((c) => "Some Tab" === attr(c, "title"))).toBeTruthy(
      "Could not find tab Some Tab"
    );
  });
  ...
  it("should render the field labels based on show_condition", () => {
    ...
    expect(
      headers.find(
        (r) => r.nativeElement.textContent.trim() === "Complex field"
      )
    ).toBeFalsy("Found row with label Complex field");
    expect(
      headers.find((r) => r.nativeElement.textContent.trim() === "Last name")
    ).toBeTruthy("Cannot find row with label Last name");
    expect(
      headers.find((r) => r.nativeElement.textContent.trim() === "First name")
  ...
  it("should render a row for each field in a given tab", () => {
    let rows = de.query($NAME_TAB_CONTENT).queryAll(By.css("tbody>tr"));
  ...
  it("should render each simple field label as a table header", () => {
    ...
    SIMPLE_FIELDS.forEach((field) => {
      expect(
        headers.find((r) => r.nativeElement.textContent.trim() === field.label)
    ...
    FIELDS.forEach((field) => {
      expect(
        readFields.find((f) => {
          ...
          return (
            JSON.stringify(fieldInstance.caseField) === JSON.stringify(field)
          );
        })
  ...
  it("should change button label when notified about callback errors", () => {
    let callbackErrorsContext: CallbackErrorsContext =
      new CallbackErrorsContext();
    callbackErrorsContext.trigger_text =
      CaseFullAccessViewComponent.TRIGGER_TEXT_START;
    ...
    expect(eventTrigger.triggerText).toEqual(
      CaseFullAccessViewComponent.TRIGGER_TEXT_START
    );

    callbackErrorsContext.trigger_text =
      CaseFullAccessViewComponent.TRIGGER_TEXT_CONTINUE;
    component.callbackErrorsNotify(callbackErrorsContext);
    fixture.detectChanges();
  ...
  it("should clear alerts and errors when selected trigger changed", () => {
    ...
    const httpError = HttpError.from(
      new HttpErrorResponse({ error: VALID_ERROR })
    );
...
xdescribe("CaseFullAccessViewComponent - no tabs available", () => {
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      imports: [PaletteUtilsModule, MatTabsModule],
      declarations: [
        CaseFullAccessViewComponent,
        LabelSubstitutorDirective,
        DeleteOrCancelDialogComponent,
